...
CTX=kind-hd1; NS=hugegraph; REL=hd1; cd ~/hd1; rm -f D3Q-DONE
K="kubectl --context $CTX -n $NS"
{
echo "=== $(date -Is) D3Q start"
...
sleep 3
# sampler: every 2 s
( while :; do T=$(date +%s.%N); R=$(curl -s -m 2 -o /dev/null -w "%{http_code}" http://127.0.0.1:18621/v1/ready); H=$(curl -s -m 2 -o /dev/null -w "%{http_code}" http://127.0.0.1:18621/v1/health); G=$(curl -s -m 2 http://127.0.0.1:18621/actuator/prometheus | grep -E "^hg_raft_(has_leader|alive_peers)" | awk "{printf \"%s=%s \", \$1, \$2}"); RC=$($K get pod hd1-hugegraph-pd-0 -o jsonpath="{.status.conditions[?(@.type==\"Ready\")].status}" 2>/dev/null); echo "$T ready=$R health=$H ${G}podReady=$RC"; sleep 2; done ) > d3q-samples.log 2>&1 & SAMP=$!
sleep 8
...
for i in $(seq 1 120); do
  ...
  S2=$($K get pod hd1-hugegraph-store-2 -o jsonpath="{.status.phase}/{.status.initContainerStatuses[0].state | keys[0]}/{.status.containerStatuses[0].ready}" 2>/dev/null)
  echo "T0+$(( $(date +%s) - T0 ))s pdReady=[$READY] store2=$S2"
  [[ "$READY" == "True True True " && "$S2" == *"/true" ]] && break
  sleep 5
done
...
echo "--- sample summary"; N=$(wc -l < d3q-samples.log); R503=$(grep -c "ready=503" d3q-samples.log); R200=$(grep -c "ready=200" d3q-samples.log); H200=$(grep -c "health=200" d3q-samples.log); HN=$(grep -vc "health=200" d3q-samples.log); echo "samples=$N ready503=$R503 ready200=$R200 health200=$H200 healthNot200=$HN"
F503=$(grep -m1 "ready=503" d3q-samples.log | cut -d" " -f1); L503=$(grep "ready=503" d3q-samples.log | tail -1 | cut -d" " -f1); echo "first503=$F503 last503=$L503 T0=$T0 (offsets: $(python3 -c "print(round(${F503:-0}-$T0,1), round(${L503:-0}-$T0,1))" 2>/dev/null))"
echo "podReady=False samples: $(grep -c "podReady=False" d3q-samples.log); has_leader=0 samples: $(grep -c "hg_raft_has_leader=0" d3q-samples.log)"
echo "=== $(date -Is) D3Q end"; touch D3Q-DONE
} > d3q.log 2>&1
